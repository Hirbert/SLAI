...
#    Needleman_Wunsch(seq1,seq2,score_matrix)
    ...
    while True:
        if not m_score:
            return str(m_score),m_cigar,str(start1),str(end1),mseq1,str(start2),str(end2),mseq2,maxx,maxy,matchsitelist,similarity
        if m_score < 0:
            score_matrix[maxx][maxy]=-1
            zero_site.append([maxx,maxy])
            (m_score,m_cigar,start1,end1,mseq1,start2,end2,mseq2,maxx,maxy,matchsitelist,similarity)=Smith_Waterman(seq1,seq2,score_matrix)
        elif keysite1 and keysite2:
            if [keysite1,keysite2] in matchsitelist:
#                return m_score,m_cigar,start1,end1,mseq1,start2,end2,mseq2
                return str(m_score),m_cigar,str(start1),str(end1),mseq1,str(start2),str(end2),mseq2,maxx,maxy,matchsitelist,similarity
            else:
                for each in matchsitelist:
                    maxx=each[0]
                    maxy=each[1]
                    score_matrix[maxx][maxy]=-1
                    zero_site.append([maxx,maxy])
